...
function draw() {
  ...
  if (retração && interpolação > 0) {
   ...
   interpolação += 0.0001; // Expansão suave
  }
  ...
  vooIniciado = true;
  passaroTocou = true; // só inicia uma vez
}
...
function Arvore(tam) {
  ...
  //Controla a expessura do tronco. 
    especura = map(tam, 15, tamMIM, 0.5,40);
...
if(!fimDaChuva){
  ...
  passaroX = lerp(passaroX, alvoX, 0.01);
   
  desenharPassaro(passaroX, passaroY, passaroEscala);

  // Quando estiver suficientemente perto, parar
  if (dist(passaroX, passaroY, alvoX, alvoY) < 3) {
    vooIniciado = false;
    passaroX = alvoX;
    passaroY = alvoY;
  }
